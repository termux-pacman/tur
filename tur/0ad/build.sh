...
termux_step_make_install() {
	...
	install -Dm755 $TERMUX_PKG_SRCDIR/binaries/system/libCollada.so $TERMUX_PREFIX/lib/0ad/libCollada.so

	# Install internally built Spidermonkey library
	install -Dm755 $TERMUX_PKG_SRCDIR/binaries/system/libmozjs128-release.so $TERMUX_PREFIX/lib/0ad/libmozjs128-release.so

	# Install desktop resources
	local RES_DIR="$TERMUX_PKG_SRCDIR/build/resources"
	install -Dm644 "$RES_DIR/0ad.desktop" "$TERMUX_PREFIX/share/applications/0ad.desktop"
	install -Dm644 "$RES_DIR/0ad.png" "$TERMUX_PREFIX/share/icons/hicolor/128x128/apps/0ad.png"
	install -Dm644 "$RES_DIR/0ad.appdata.xml" "$TERMUX_PREFIX/share/metainfo/0ad.appdata.xml"
	install -Dm644 "$RES_DIR/pyrogenesis.xml" "$TERMUX_PREFIX/share/mime/packages/pyrogenesis.xml"
